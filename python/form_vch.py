...
def run_epson_scan(kode_voucher, waktu_sekarang):
    ...
    window_epson.wait("exists", timeout=30)  # Sesuaikan waktu tunggu jika diperlukan
    
    # Klik tombol "Customize"
    button_customize = window_epson.child_window(auto_id="1017", control_type="Button")
...
def submit_form():
    ...
    if confirm:
        ...
        kode_voucher = entry_kode_voucher.get()
        waktu_sekarang = datetime.now().strftime("%d-%m-%Y %H:%M:%S")
        app_epson = Application(backend="uia")
        app_epson.start(r"C:\Windows\twain_32\escndv\escndv.exe")
